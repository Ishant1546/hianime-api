...
export const extractStream = async ({ selectedServer, id }) => {
  ...
  // hianime.ad format: server has embedUrl directly (no AJAX sources step needed)
  if (selectedServer.embedUrl && !selectedServer.id) {
    return {
      id,
      type: selectedServer.type,
      link: { file: selectedServer.embedUrl, type: 'embed' },
      tracks: [],
      intro: null,
      outro: null,
